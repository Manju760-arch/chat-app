...
type="text" className='p-2 border border-gray-500 rounded-md focus:outline-none' placeholder='Full Name' required />
          ...
          <div className='flex flex-col gap-2'>
            {currState==="Sign up"?(
              <p className='text-sm text-white-600'>Already have an account?<span onClick={()=>{setCurrState("Login"); setIsDataSubmitted(false)}} className='font-medium text-darkviolet-500 cursor-pointer'>Login here</span></p>
            ):(
              <p className='text-sm text-white-600'>Create an Account <span onClick={()=>setCurrState("Sign up")} className='font-medium text-darkviolet-500 cursor-pointer'>Click here</span></p>
            )}
          </div>
      </form>
    </div>
